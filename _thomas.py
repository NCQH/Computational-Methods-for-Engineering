...
def _lu_factorization_thomas(A, transfered=False):
    if not transfered:
...
def _solve_thomas_algorithm(A, B_t, transfered=False):
    if not transfered:
        A = _matrix_to_thomas_form(A)
    if A.shape[1] != 3:
        return 'Error: Input or transpose matrix error'
...
def _inverse_thomas_algorithm(A):
    A = np.array(A, dtype=float)
    I = np.eye(A.shape[0])

    return _solve_thomas_algorithm(A, I, transfered=False)

def _determinant_thomas_algorithm(A):
    A = np.array(A, dtype=float)
    A = _matrix_to_thomas_form(A)
    A = _thomas_matrix_transform(A)
    det = 1
